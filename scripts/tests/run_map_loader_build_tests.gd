extends Node
## Runs GUT with map loader + build integration tests.
## Use as main scene or run with "Run Current Scene" to test map load flow in-editor.
...
func _ready() -> void:
	...
	cfg.options.dirs = []
	cfg.options.tests = ["res://scripts/tests/test_map_loader_build.gd"]
