...
const PREFAB_MAP: Dictionary = {
	"enemy_standard": "res://scenes/ecs/e_enemy_standard.tscn",
	"enemy_laser_immune": "res://scenes/ecs/e_enemy_laser_immune.tscn",
	"enemy_physical_immune": "res://scenes/ecs/e_enemy_physical_immune.tscn",
	"enemy_saboteur": "res://scenes/ecs/e_enemy_saboteur.tscn",
	"enemy_commander": "res://scenes/ecs/e_enemy_commander.tscn"
}
...
func _spawn_enemy() -> void:
	...
	_spawn_ecs_enemy(enemy_id, spawn_position, health_multiplier, speed_multiplier)


func _spawn_ecs_enemy(enemy_id: String, spawn_position: Vector3, health_multiplier: float, speed_multiplier: float) -> void:
	var prefab_path: String = PREFAB_MAP.get(enemy_id, PREFAB_MAP[DEFAULT_ENEMY_ID])
	var entity_scene: PackedScene = load(prefab_path) as PackedScene
	if entity_scene == null:
		push_warning("ECS enemy scene not loaded: %s" % prefab_path)
		return
	var entity: Node = entity_scene.instantiate()
	if entity == null:
		return
	var body: CharacterBody3D = entity.get_node_or_null("ShipBody") as CharacterBody3D
	if body == null:
		for child in entity.get_children():
			if child is CharacterBody3D:
				body = child as CharacterBody3D
				break
	if body == null:
		push_warning("ECS enemy prefab has no ShipBody (CharacterBody3D): %s" % enemy_id)
		entity.queue_free()
		return
	var main: Node = get_tree().root.get_node_or_null("Main")
	var enemies_parent: Node = main.get_node_or_null("Enemies") if main else null
	if enemies_parent:
		enemies_parent.add_child(entity)
	body.global_position = spawn_position
	# Apply wave multipliers to baked component values
	ECS.world.add_entity(entity, [], false)
	_apply_wave_multipliers(entity, health_multiplier, speed_multiplier)
	_active_enemies.append(body)
	enemy_spawned.emit(body)


func _apply_wave_multipliers(entity: Node, health_mult: float, speed_mult: float) -> void:
	if not entity.has_method("get_component"):
		return
	var c_health: C_Health = entity.get_component(C_Health) as C_Health
	if c_health:
		c_health.maximum *= health_mult
		c_health.current = c_health.maximum
	var c_state: C_EnemyState = entity.get_component(C_EnemyState) as C_EnemyState
	if c_state:
		c_state.speed *= speed_mult
