extends System
class_name SaboteurMovementSystem
## Saboteur movement: fly toward target_position (power line midpoint) with saboteur-specific steering.
const ObstacleDebugStoreClass = preload("res://scripts/debug/obstacle_debug_store.gd")


func query() -> QueryBuilder:
	return q.with_all([C_SaboteurState, C_SaboteurMovement, C_Targeting, C_Transform3D, C_PhysicsBodyRef, C_EnemyState])
...
func _process_entity_movement(entity: Entity, delta: float) -> void:
	...
	if c_state.is_destroyed or c_body_ref == null or c_body_ref.body == null or c_movement == null or c_saboteur == null:
		return
	...
	if dist_to_target <= c_movement.safe_distance:
	...
	if dist_to_target < c_movement.approach_decel_dist:
		var decel_range: float = maxf(c_movement.approach_decel_dist - c_movement.safe_distance, 0.01)
		var t: float = clampf((dist_to_target - c_movement.safe_distance) / decel_range, 0.0, 1.0)
		approach_mult = lerpf(c_movement.min_approach_speed, 1.0, t)
	...
	if closest_obstacle_dist < c_movement.obstacle_slow_dist:
		...
		obstacle_mult = lerpf(c_movement.min_speed_near_obstacle, 1.0, t)
	var speed_mult: float = minf(approach_mult, obstacle_mult)
...
func _compute_desired_direction(body: CharacterBody3D, c_targeting: C_Targeting) -> Vector3:
	...
	if c_targeting and _has_valid_target_position(c_targeting):
		target_pos = c_targeting.target_position
	elif c_targeting and c_targeting.target_node != null and is_instance_valid(c_targeting.target_node):
...
func _has_valid_target_position(c_targeting: C_Targeting) -> bool:
	if c_targeting.target_position.x == INF or c_targeting.target_position.x == -INF:
		return false
	return true
